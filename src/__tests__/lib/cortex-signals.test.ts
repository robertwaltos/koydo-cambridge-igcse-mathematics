...
import { emitPrismSignal } from "@/lib/cambridge-igcse-mathematics/cortex-signals";

describe("PRISM Signals", () => {
  it("dispatches custom event on window", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    emitPrismSignal({ eventType: "question_answered", activityType: "cambridge-igcse-mathematics" });
  ...
  it("includes correct event detail", () => {
    ...
    emitPrismSignal({ eventType: "exam_started", value: 1, activityType: "cambridge-igcse-mathematics" });
    expect(detail).toEqual({ eventType: "exam_started", value: 1, activityType: "cambridge-igcse-mathematics" });
