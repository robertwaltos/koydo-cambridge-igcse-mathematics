...
import { EXAM_CONFIG } from "@/lib/cambridge-igcse-mathematics/config";

describe("Cambridge IGCSE Mathematics Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM108");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("cambridge-igcse-mathematics");
  });
